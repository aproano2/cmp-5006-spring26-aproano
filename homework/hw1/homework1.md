...
Modular Arithmetic: Find $x$ for the following:
$x \equiv 457 \pmod{13}$
$x \equiv -25 \pmod{7}$
...
Information Theory: A source produces ${A, B, C, D}$ with probabilities $P(A)=1/2$, $P(B)=1/4$, $P(C)=1/8$, and $P(D)=1/8$.
Calculate the Shannon Entropy $H(X)$.
Bayes' Theorem: An Intrusion Detection System (IDS) is designed to flag a "Brute Force" attack.
